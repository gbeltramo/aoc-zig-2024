...
const input_path_parts = [3][]const u8{ "puzzles", "10", "input.txt" };
// const input_path_parts = [3][]const u8{ "puzzles", "10", "example-input.txt" };
...
fn sortAssumingAllRulesPresent(numbers: std.ArrayList(u16), rules: std.AutoHashMap(OrderRule, u8)) !void {
    var numbers_are_unsorted: bool = true;
    sort_outer_loop: while (numbers_are_unsorted) {
        for (0..numbers.items.len - 1) |idx0| {
            ...
            for (0..idx0) |idx_before| {
                ...
                if (maybe_bad != null) {
                    try io.stderr.print("\t\tDEBUG: reordering ({d}, {d})\n", .{ before_value, value });
                    numbers.items[idx_before] = value;
                    numbers.items[idx0] = before_value;
                    continue :sort_outer_loop;
                }
            ...
            for (idx0 + 1..numbers.items.len) |idx_after| {
                ...
                if (maybe_bad != null) {
                    try io.stderr.print("\t\tDEBUG: reordering ({d}, {d})\n", .{ value, after_value });
                    numbers.items[idx0] = after_value;
                    numbers.items[idx_after] = value;
                    continue :sort_outer_loop;
                }
...
pub fn main() !void {
        ...
        for (0..numbers.items.len - 1) |idx0| {
            ...
            for (0..idx0) |idx_before| {
                ...
                if (maybe_bad != null) {
                    ...
                    try sortAssumingAllRulesPresent(numbers, rules);
    ...
    try io.stdout.print("INFO: The solution to puzzle 10 is: {d}\n", .{total});
