...
pub fn main() !void {
    ...
    var cwd = std.fs.cwd();
    const things_to_join = [2][]const u8{ "puzzles", "06" };
    ...
    for (0..content.len) |idx| {
        ...
        if (five_utils.isDo(content, idx)) {
            do_add = true;
        }

        if (five_utils.isDont(content, idx)) {
            do_add = false;
        }

        if (do_add and five_utils.isStartMul(content, idx)) {
    ...
    try io.stdout.print("INFO: The solution to puzzle 06 is: {d}\n", .{total});
