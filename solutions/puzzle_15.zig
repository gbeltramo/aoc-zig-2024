...
const input_path_parts = [3][]const u8{ "puzzles", "15", "input.txt" };
// const input_path_parts = [3][]const u8{ "puzzles", "15", "example-input.txt" };
...
pub fn main() !void {
    ...
    // Comment: used for pruning nested loop below
    const half_num_rows: T_index = @intCast(@divTrunc(num_rows, 2) + 1);
    ...
    while (antennas_iter.next()) |entry| {
        ...
        for (0..value_list.items.len) |idx_i| {
            for (idx_i + 1..value_list.items.len) |idx_j| {
                ...
                delta_row = c1.row_i - c0.row_i;

                // Comment: pruning
                if (delta_row > half_num_rows) {
                    continue;
                }
                ...
                if (c1.col_i > c0.col_i) {
                    delta_col = c1.col_i - c0.col_i;

                    // Comment: case \, antinote above c0
                    if ((delta_row <= c0.row_i) and (delta_col <= c0.col_i)) {
                    ...
                    if (((delta_row + c1.row_i) < num_rows) and ((delta_col + c1.col_i) < num_cols)) {
                    ...
                    delta_col = c0.col_i - c1.col_i;

                    // Comment: case /, antinote above c0
                    if ((delta_row <= c0.row_i) and ((delta_col + c0.col_i) < num_cols)) {
                        antinote_row_0 = c0.row_i - delta_row;
                        antinote_col_0 = c0.col_i + delta_col;
                        const index = antinote_row_0 * num_cols + antinote_col_0;
                        if (antinodes_locations.data[index] == 0) {
                    ...
                    // Comment: case /, antinote below c1
                    if (((delta_row + c1.row_i) < num_rows) and (delta_col <= c1.col_i)) {
                        antinote_row_1 = c1.row_i + delta_row;
                        antinote_col_1 = c1.col_i - delta_col;
                        const index = antinote_row_1 * num_cols + antinote_col_1;
                        if (antinodes_locations.data[index] == 0) {
    ...
    try io.stdout.print("INFO: The solution to puzzle 15 is: {d}\n", .{total});
    // <<<
